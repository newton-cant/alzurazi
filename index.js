...
const { prefi, ownerName, ownerNumber, cdd, ammOff, cr, crfig, crlv, crtt, crh } = infos
...
async function starts() {
	...
	supra.on('connecting', () => {
		start('2', 'AL-ZURAS CONNECTING...')
	})
	supra.on('open', () => {
		success('2', 'AL-ZURAS CONECTADO')
	})
	await supra.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./BarBar.json', JSON.stringify(supra.base64EncodedAuthInfo(), null, '\t'))
/*
supra.on('group-participants-update', async(chat) => {
		if (!welkom.includes(chat.jid)) return
        try {
            mem = chat.participants[0]
            console.log(chat)
            try {
                var pp_user = await supra.getProfilePicture(mem)
            } catch (e) {
                var pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
           }
            if (chat.action == 'add') {
                ini_user = supra.contacts[mem]
                ini_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=b5ca61b0c4c12d316ab1fce8&img=${pp_user}&text=${ini_user.notify}`)
                group_info = await supra.groupMetadata(chat.jid)
                welkam = `*〔 WELCOME IN GC ${group_info.subject} 〕*\n╭━━━━━━━━━━━━━━━━━━━━ \n┃ *➣ Nama   :* \n┃ *➣ Umur   :* \n┃ *➣ Askot   :* \n┃ *➣ Gender :* \n┃ *➣ Insta    :* \n╰━━━━━━━━━━━━━━━━━━━━\n @${mem.split('@')[0]} Intro/Dikick!!!`
                supra.sendMessage(chat.jid, ini_img, MessageType.image, { caption: welkam, contextInfo: {"mentionedJid": [mem]} })
            }
            try {
                var pp_usere = await supra.getProfilePicture(mem)
            } catch (e) {
                var pp_usere = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
           }
            if (chat.action == 'remove') {
                ini_user = supra.contacts[mem]
                group_indo = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=b5ca61b0c4c12d316ab1fce8img=${pp_usere}&text=${ini_user.notify}`)
                ini_out = `*SELAMAT TINGGAL @${mem.split('@')[0]}* \nJasamu akan saya kubur dalam dalam`
                supra.sendMessage(chat.jid, group_indo, MessageType.image, {caption: ini_out, contextInfo: {"mentionedJid": [mem]}})
            }
        } catch (e) {
            console.log('Error :', e)
        }
...
╚════════════════❍
Está registrado no AL-ZURAS, ${prefix}menu para ver minha lista de comandos`
...
break
 ...
 link: ${anu.link}`
    ...
    play = body.slice(8)
      reply('aguarde um pouco punheteiro...')
      anu = await fetchJson(`https://mnazria.herokuapp.com/api/porn?search=${play}`)
      teks = `        *AL-ZURAS XVIDEOS*\n      
...
case 'igtraduzir': //@SUPRA ♡
 if (args.length < 1) return reply(`Insira o texto em português que você deseja traduzir para inglês`)
 supra.updatePresence(from, Presence.composing)
 tels = body.slice(12)
 try {
 anu = await fetchJson(`https://docs-jojo.herokuapp.com/api/translate?text=${tels}&from=id&to=ig`, {
 method: 'get'
  })
 reply(anu.translated_text)
 } catch {
...
tels = body.slice(4)
try {
anu = await fetchJson(`https://brasilapi.com.br/api/ddd/v1/93`, { method: 'get' })
ccg =
` INFORMAÇÕES DO DDD
  ‣ Estado: ${anu.state}
  ‣ Cidades: 
    ${anu.cities}\n`
supra.sendMessage(from, ccg, text, {quoted:mek})
} catch {
reply(mess.ferr)
}
break
...
case 'sticker1': //@SUPRA ♡
 ...
 case 'stickerwa': //@SUPRA ♡
  if (isBanned) return reply('Você foi banido')
    ...
    query = args.join(" ")
    get_result = await fetchJson(`https://api.lolhuman.xyz/api/stickerwa?apikey=${up.apikey}&query=${query}`)
...
case 'toimg':
  ...
  case 'bass':  
   ...
   if (isBanned) return reply('Você foi banido')
		if (args.length < 1) return reply('Qual o nome da música?')
         ...
         supra.sendMessage(from, buffer, image, {quoted: mek, caption:'AL-ZURAS ESTÁ BAIXANDO SUA MÚSICA🎶🎶'})
 ...
 break 
  ...
  if (isBanned) return reply('Você foi banido') 
   ...
   supra.sendMessage(from, buffer, image, {quoted: mek, caption:'AL-ZURAS ESTÁ BAIXANDO SEU VÍDEO🎬'})
   ...
   case 'xvideosd':
  play = body.slice(8)
  anu = await fetchJson(`http://fahmiapi.herokuapp.com/nsfw/xvideos?url=${play}&apikey=${up.apikey2}`)
   buffer = await getBuffer(anu.streams)
   attp2 = ` título = ${anu.title}
             baixar = ${anu.lq}`
   supra.sendMessage(from, attp2, text, {quoted: mek})
		    ...
		    supra.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
		      ...
		      setTimeout( () => {
                   ...
                   supra.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                   	...
                   	if (budy.includes(";-;")){
			        supra.updatePresence(from, Presence.composing)
			        reply("isso é cringe mn")
			        }			       
                    if (budy.includes("al-zuras")){
		           supra.updatePresence(from, Presence.composing)
